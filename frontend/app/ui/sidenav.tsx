'use client'; // usePathname kullandığımız için bu sayfanın Client tarafında çalışması gerekir.

import Link from 'next/link';
import { usePathname } from 'next/navigation';

// Menü elemanlarımızı temiz bir liste halinde tutuyoruz
const links = [
    {
        name: 'Genel Bakış',
        href: '/',
...
export default function SideNav() {
    // Mevcut sayfanın URL yolunu (pathname) alıyoruz
    ...
    return (
        <div className="flex h-full flex-col px-3 py-4 md:px-2 bg-white border-r border-gray-200 shadow-sm">
            ...
            <div className="flex flex-row space-x-2 md:flex-col md:gap-2 py-1 md:space-x-0 flex-grow">
                {links.map((link) => {
                    // Bulunduğumuz sayfa, bu linkin sayfasına eşit mi diye kontrol ediyoruz
                    ...
                    return (
                        <Link
                            ...
                            className={`group flex h-[48px] grow items-center justify-center gap-3 rounded-xl p-3 text-sm font-medium transition-all md:flex-none md:justify-start md:px-4 border ${isActive
                                    ? 'bg-blue-50 text-blue-700 border-blue-200 shadow-sm' // Aktif Sayfa Stili (Senin istediğin Mavi Kutu / Focus Ring)
                                    : 'bg-transparent text-slate-600 border-transparent hover:bg-slate-50 hover:text-slate-900' // Normal Sayfa Stili
                                }`}
                        >
