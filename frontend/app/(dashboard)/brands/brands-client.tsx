...
export default function BrandsClient() {
  ...
  const fetchBrands = async () => {
    ...
    setError(null);
    try {
      const res = await authFetch(`${API_BASE_URL}/api/Brand`);
      if (res.ok) {
        const data = await res.json();
        setBrands(Array.isArray(data) ? data : data.items || []);
      } else {
        setError('Markalar yüklenirken sorun oluştu.');
      }
    } catch (err) {
  ...
  const handleAddBrand = async (e: React.FormEvent) => {
    ...
    try {
      const res = await authFetch(`${API_BASE_URL}/api/Brand`, {
        ...
        body: JSON.stringify({ brandName: newBrandName }) // Assume CreateBrandDto has brandName
      });
      if (res.ok) {
        setNewBrandName('');
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <h2 className="text-lg font-bold text-slate-800">Yeni Marka Ekle</h2>
