...
export default function UrunEnvanterSayfasi() {
    ...
    // Backend pagination desteklediği için ürünleri doğrudan kullanıyoruz
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-[70] overflow-y-auto bg-slate-900/50 backdrop-blur-sm">
                    <div className="flex items-center justify-center min-h-screen p-4">
                        <form onSubmit={handleSubmit(onSubmit)} className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-4xl relative transition-colors">
                            <input type="hidden" {...register("id")} />
                            <div className="px-8 py-5 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/50 rounded-t-2xl">
                                <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">{watch("id") ? "Düzenle" : "Yeni Ürün Kartı"}</h2>
                                <button type="button" onClick={() => setIsModalOpen(false)} className="text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300">
                            ...
                            <div className="px-8 py-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/80 flex justify-end gap-3 rounded-b-2xl">
                                <button type="button" onClick={() => setIsModalOpen(false)} className="px-5 py-2.5 text-slate-600 dark:text-slate-300 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-600 rounded-xl font-medium text-sm transition-colors">İptal Et</button>
                                <button type="submit" disabled={isSubmitting} className="px-5 py-2.5 text-white bg-blue-600 hover:bg-blue-700 shadow-md shadow-blue-600/20 rounded-xl font-medium text-sm disabled:opacity-70 flex items-center gap-2 transition-colors">
